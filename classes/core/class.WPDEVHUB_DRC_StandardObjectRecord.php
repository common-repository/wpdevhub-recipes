 ...
 */
class WPDEVHUB_DRC_StandardObjectRecord{
    ...
    */
    ...
    public static function getTableName(){
        global $wpdb;
        $name = $wpdb->base_prefix . WPDEVHUB_CONST_DRC_DB_PREFIX . '_' . static::TABLE_NAME;
    ...
    public function updateSlug($string){
        $newSlug = WPDEVHUB_DRC_Utilities::buildSlug($string);
    ...
    public static function createNewSlug($string){
        ...
        while($slugAvailable == false){
            $counter++;
            $newSlug = WPDEVHUB_DRC_Utilities::buildSlug($string." ".$counter);
            $slugAvailable = static::isSlugAvailable($newSlug);
        }
        return $newSlug;
    }
    ...
    public function buildHashKey($maxAttempts = 25){
        ...
        while($goodHash == false){
            $hashKey = WPDEVHUB_DRC_Utilities::generateSalt('', static::HASH_KEY_LENGTH, false);
            ...
            if($attempts > $maxAttempts){
                WPDEVHUB_DRC_Utilities::logError(__CLASS__."::".__FUNCTION__." :: Could not create new Hash Key, attempts timed out.");
                return false;
            }
        }
        return true;
    ...
    public static function buildShortcodeHelper($params){
        ...
        $html = '['.WPDEVHUB_CONST_DRC_SLUG.$paramString.']';
        return $html;
    }
    ...
    public static function getFormattedStatusImage($status, $width='32px'){
        $statusCollection = array(
            self::STATUS_ACTIVE=>'<img src="'.WPDEVHUB_CONST_DRC_URL_IMAGES.'/accept.png" style="width:'.$width.';" title="Active" />',
            self::STATUS_INACTIVE=>'<img src="'.WPDEVHUB_CONST_DRC_URL_IMAGES.'/cancel.png" style="width:'.$width.';" title="Inactive" />',
        );
    ...
    */
    ...
    public static function formatTimestampForSite($timestamp){
        ...
        //WPDEVHUB_DRC_Utilities::logMessage("Timezone setting: ".$timezone_str);
        if(empty($timezone_str)){
            $gmt_offset = get_option('gmt_offset');
            //WPDEVHUB_DRC_Utilities::logMessage("GMT Offset setting: ".$gmt_offset);
            if(empty($gmt_offset)){
                $gmt_offset = 0;
                //WPDEVHUB_DRC_Utilities::logMessage("RESETTING GMT OFFSET".$gmt_offset);
            }
            $timezone_str = WPDEVHUB_DRC_Utilities::gmtOffsetToTimezoneName($gmt_offset);
            //WPDEVHUB_DRC_Utilities::logMessage("Timezone String From GMT Offset: ".$timezone_str);
        }

        if(empty($timezone_str)){
            //WPDEVHUB_DRC_Utilities::logMessage("Timezone String Was Empty: ".$timezone_str);
            $timezone_str = 'UTC';
        }
        ...
        //WPDEVHUB_DRC_Utilities::logMessage("Final Timezone setting: ".$timezone_str);
    ...
    public static function filterObjectsByFieldValue($objects, $fieldKey, $fieldValue){
        ...
        foreach($objects as $arrayKey=>$object){
            //WPDEVHUB_DRC_Utilities::logMessage("Inside filterObjectsByFieldValue Set1[".$object->$fieldKey."] Set2[$fieldValue]");
    ...
    public static function checkForDeleteRequest($options){
        ...
        $return = false;
        //WPDEVHUB_DRC_Utilities::logMessage("Delete(".$requestVars['delete'].") ID(".$requestVars['id'].")");
    ...
    public function prepareDeleteConfirmation($type){
        ...
        $msg = '';
        $msg .= '
			<div id="confirmationDeleteDialog" class="drc-pbox3" style="">
				<img src="'.WPDEVHUB_CONST_DRC_URL_IMAGES.'/error.png" style="float:left; width:50px; padding-right:25px;" />
					...
					<form method="post" action="#">
						...
						<input type="submit" class="drc-button" name="submit" value="Confirm Delete" />
    ...
    public function processDeleteRequest($requestVars){
        //WPDEVHUB_DRC_Utilities::logMessage("Inside ".__CLASS__.__FUNCTION__);
        if(!isset($requestVars['deleteHash'])){
            //WPDEVHUB_DRC_Utilities::logError('Delete Attempt made without a Delete Hash');
            return '';
        }

        $hash = $this->getDeleteObjectHash();
        if($requestVars['deleteHash']!=$hash){
            //WPDEVHUB_DRC_Utilities::logError('Delete Attempt made with invalid Delete Hash Passed('.$requestVars['deleteHash'].') Generated('.$hash.')');
            return '';
        }
        ...
        try{
            ...
            //WPDEVHUB_DRC_Utilities::logError("Error Caught Processing Delete command - were we acting upon the Standard Object? Message: ".$e->getMessage());
        }
    ...
    public function getUserName(){
        ...
        $userName = WPDEVHUB_DRC_Utilities::getFromObject('user_nicename', $userObject);
        return $userName;
    }
