 ...
 */
class WPDEVHUB_DRC_StandardManager{

    public static function buildManagerTable($rows){
        //WPDEVHUB_DRC_Utilities::logMessage("Manager Table Rows: ".print_r($rows,true));
        /*
          $row = array();
          $row[] = array(
              'title'='Edit',
        ...
        $html = '';
        if(count($rows)==0){
            $html .= '<p style="text-align:center;"><img src="'.WPDEVHUB_CONST_DRC_URL_IMAGES.'/no_saved_objects.png" /></p>';
        }else{

            //Get Headers:
            $headers = array();
            foreach($rows[0] as $hk=>$headerCell){
                //WPDEVHUB_DRC_Utilities::logMessage("HEADER CELL: HK($hk) ".print_r($headerCell, true));
                if($hk === 'onclick' || $hk === 'style'){
                }else{
                    $headers[]=$headerCell['title'];
                }
            }
            //WPDEVHUB_DRC_Utilities::logMessage("HEADERS: ".print_r($headers, true));
            $html .= '';
            $html .= '<table id="managerTable_'.$rand.'" class="drc-content-manager" style="clear:both; width:100%;">';
            $html .= '<thead>';
            $html .= '<tr>';
