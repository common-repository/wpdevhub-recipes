 ...
 */
class WPDEVHUB_DRC_StandardEditor{
    ...
    const ET_DATE_TIME = 25;        // In order to enable Date Time it has to be included
        //wp_enqueue_script( WPDEVHUB_CONST_DRC_SLUG.'jquery-time-picker' ,  WPDEVHUB_CONST_DRC_URL.'/js/jquery-ui-timepicker-addon.min.js',  array('jquery' ));
    const ET_SKIP = 26;
    ...
    public static function buildPageTemplate($class, $editorParams=array(), $forceEditor=false){
        ...
        //WPDEVHUB_DRC_Utilities::logMessage("Request Vars: ".print_r($_REQUEST, true));
        ...
        $id = WPDEVHUB_DRC_Utilities::getRequestVarIfExists('id');
    ...
    public static function buildEditor($options, $target, $id="", $forceDemoOnly=false){
        ...
        if(empty($tabRows)){
            $html .= '<table class="drc-content-editor">';
            foreach($options as $optionRow){
                if($forceDemoOnly){
            ...
            foreach($options as $optionRow){
                if(array_key_exists('rowType', $optionRow) && $optionRow['rowType']=='tabHeader'){
                    ...
                    $html .= '<table class="styledTable1 drc-content-editor">';
                    $firstDiv = false;
                }else{
                    $html .= self::buildEditorRow($optionRow);
                }
    ...
    public static function buildEditorRow($options=array()){
        ...
        if($options['formType'] == self::ET_HIDDEN){
            ...
            $html .= '<tr>';
            $html .= '<td>';
            $html .= '<div class="drc-content-editor-title">'.$options['title'].'</div>';
            $html .= '<div class="drc-content-editor-help">'.$options['help'].'</div>';
            $html .= '</td>';
            $html .= '<td>';
    ...
    public static function buildEditorCell($options){
        ...
        switch ($options['formType']){
            case self::ET_TEXT:
                ...
                //WPDEVHUB_DRC_Utilities::logMessage("Common Editor TEXT: Value:".print_r($options['value'],true));
                break;
            ...
            case self::ET_SLUG_EDITOR:
                $html .= '
					...
					<br /><a href="javascript:WPDEVHUB_DRC_Admin.createSlug(\''.$options['objectName'].'\',\''.$options['sourceName'].'\',\''.$options['slugClassName'].'\');">create from title</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:WPDEVHUB_DRC_Admin.validateSlug(\''.$options['objectName'].'\',\''.$options['slugClassName'].'\');">validate slug</a>';
                break;
            case self::ET_TEXT_ADDITIONAL:
                ...
                if(WPDEVHUB_DRC_Utilities::validateArray($options['value'])){
                    foreach($options['value'] as $key=>$value){
                        $html .= '<div id="'.$options['objectName'].$key.'_wrapper"><input type="text" id="'.$options['objectName'].$key.'" name="'.$options['objectName'].$key.'" value="'.$value.'" size="'.$size.'"  /><button type=button onclick="WPDEVHUB_DRC_Admin.deleteOptionRow(\''.$options['objectName'].$key.'\');">delete</button></div>';
                    }
                }
                $html .= '<div id="addMoreRows'.$options['objectName'].'"><a href="javascript:WPDEVHUB_DRC_Admin.addBlankRow(\'moreRows'.$options['objectName'].'\',\'addMoreRows'.$options['objectName'].'\');">add row</a></div>';
                break;
            case self::ET_TEXT_MENU_ADDITIONAL:
                ...
                if(WPDEVHUB_DRC_Utilities::validateArray($options['value'])){
                    foreach($options['value'] as $key=>$value){
                        ...
                        $html .= '</select>';
                        $html .= '<button type=button onclick="WPDEVHUB_DRC_Admin.deleteOptionRow(\''.$options['objectName'].$key.'\');">delete</button></div>';
                    }
                }
                $html .= '<div id="addMoreRows'.$options['objectName'].'"><a href="javascript:WPDEVHUB_DRC_Admin.addBlankRow(\'moreRows'.$options['objectName'].'\',\'addMoreRows'.$options['objectName'].'\');">add row</a></div>';
                break;
            ...
            case self::ET_MENU_STATUS:
                $html .= '<select id="'.$options['objectName'].'" name="'.$options['objectName'].'"> <label for="'.$options['objectName'].'"></label>';
                $html .= '<option value="">--</option>';
                foreach(WPDEVHUB_DRC_StandardObjectRecord::getAllStatusMarks() as $key=>$value){
                    $selected = '';
                    if($key==$options['value']){ $selected=' selected="selected"'; }
            ...
            case self::ET_MENU_USERID_USERNAME:
                $userArray = WPDEVHUB_DRC_Utilities::getUserLoginsAsMenuArray();
                $html .= '<select id="'.$options['objectName'].'" name="'.$options['objectName'].'"> <label for="'.$options['objectName'].'"></label>';
                $html .= '<option value="">--</option>';
                ...
                break;
            case self::ET_DATE:
                $html .= '<input type="text" id="'.$options['objectName'].'" class="WPDEVHUB_DRC_jquery-date-picker" name="'.$options['objectName'].'" value="'.WPDEVHUB_DRC_StandardObjectRecord::formatJQueryDate($options['value']).'" />';
                $html .= ' '.self::renderAjaxInsertTodaysDate($options['objectName']).' <label for="'.$options['objectName'].'"></label>';
                break;
            case self::ET_DATE_TIME:
                $html .= '<input type="text" id="'.$options['objectName'].'" class="WPDEVHUB_DRC_jquery-datetime-picker" name="'.$options['objectName'].'" value="'.WPDEVHUB_DRC_StandardObjectRecord::formatJQueryDateTime($options['value']).'" />';
                $html .= ' '.self::renderAjaxInsertTodaysDate($options['objectName']).' <label for="'.$options['objectName'].'"></label>';
                break;
            ...
            case self::ET_DPM_ANSWER_CHOICE_PICKER:
                ...
                foreach($options['formOptions'] as $key=>$value){
                    $checked = '';
                    if($value->status == WPDEVHUB_DRC_StandardObjectRecord::STATUS_ACTIVE){ $checked=' checked="checked"'; }
            ...
            case self::ET_DQQ_ANSWER_CHOICE_PICKER:
                ...
                foreach($options['value'] as $key=>$value){
                    ...
                    $html .= '<div id="'.$options['objectName'].$key.'_wrapper"><input type="radio" id="'.$options['objectName'].$key.'_rdo" name="'.$options['objectName'].'_rdo"'.$checked.' value="'.$key.'"><input type="text" name="'.$options['objectName'].$key.'_txt" value="'.$value.'" size="'.$size.'" /> <input type="button" name="'.$options['objectName'].$key.'_dlt" value="Delete" onClick="javascript:WPDEVHUB_DRC_Quiz.deleteOptionRow(\''.$options['objectName'].$key.'\')" /></div>';
                }
                $html .= '<div id="addMoreQuizRows'.$options['objectName'].'"><a href="javascript:WPDEVHUB_DRC_Quiz.setCounter('.$key.');WPDEVHUB_DRC_Quiz.addBlankRow(\''.$options['objectName'].'\', \'addMoreQuizRows'.$options['objectName'].'\');">add answer choice</a></div>';
                break;
            case self::ET_DTT_ITEMS:
                ...
                foreach($options['value'] as $key=>$value){
                    ...
                    if(is_a($value, 'WPDEVHUB_DRC_TopTen_Item')){
                        ...
                        $html .= '<button class="button" onclick="WPDEVHUB_DRC_Admin.openMediaLibrary(\''.$rowKey.'_mediaId\','.$saved_media_id.')">Open Media Library</button>';
                ...
                $html .= '<div id="addMoreDttItemRows'.$options['objectName'].'"><a href="javascript:WPDEVHUB_DRC_Admin.addDttListRow(\''.$options['objectName'].'\', \'addMoreDttItemRows'.$options['objectName'].'\');">add list item</a></div>';
                break;
            case self::ET_RADIO:
                $first = true;
            ...
            case self::ET_TEXTAREA:
                ...
                $html .= '<textarea id="'.$options['objectName'].'" name="'.$options['objectName'].'" class="drc-textarea" '.$rows.''.$cols.''.$maxlength.'>'.$options['value'].'</textarea> <label for="'.$options['objectName'].'"></label>';
                break;
            ...
            case self::ET_MEDIA_LIBRARY:
	            ...
	            $html .= '
	            <input id="'.$options['objectName'].'_button" type="button" class="drc-button button" value="Open Media Library" onclick="WPDEVHUB_DRC_Admin.openMediaLibrary(\''.$options['objectName'].'\','.$saved_value.')" />
            ...
            case self::ET_MEDIA_LIBRARY_MULTIPLE:
                ...
                $html .= '<ul id="'.$options['objectName'].'_preview" class="WPDEVHUB_DRC_jquery-ui-sortable">';
                ...
                $html .= '
	            <input id="'.$options['objectName'].'_button" type="button" class="drc-button button" value="Open Media Library" onclick="WPDEVHUB_DRC_Admin.openMediaLibraryMultiple(\''.$options['objectName'].'\')" />
	            ';
                break;
            /*
            case self::ET_BUTTON_SAVE_AND_FORWARD_ADMIN:
                ...
                if(!empty($page)){
                    $slug = WPDEVHUB_DRC_Utilities::getPageUrl($page, $params);
                    $html .= '<input type="hidden" name="forward_url_admin" value="'.$slug.'" />';
                    $html .= '<input type="submit" class="drc-button" name="submit_save_and_forward" value="Save Changes" />';
                }else{
                    WPDEVHUB_DRC_Utilities::logMessage('variable "page" not set in params for ET_BUTTON_SAVE_AND_FORWARD_ADMIN');
                }
                ...
                switch ($option['objectType']){
                    case self::OT_DATE:
                        ...
                        if(strlen(trim($enteredDate))>1){
                            $value = WPDEVHUB_DRC_StandardObjectRecord::formatIncomingDateString($requestVars[$option['objectName']]);
                        }else{
                            $value = "";
                        }
                        $processValue = true;
                    ...
                    case self::OT_BOOLEAN:
                        //WPDEVHUB_DRC_Utilities::logMessage("Inside the Editor with Object Type String");
    ...
    public static function saveEditorChanges($object, $options, $requestVars){
        ...
        WPDEVHUB_DRC_Utilities::logMessage("Inside saveEditorChanges");
        WPDEVHUB_DRC_Utilities::logMessage("Request Vars: ".print_r($requestVars, true));
        ...
        try{
            foreach($options as $option){
                ...
                if($option['title']=='ID'){
                    ...
                    WPDEVHUB_DRC_Utilities::logMessage("ObjectType Not Found in Editor: ".print_r($option, true));
                }else{
                    switch ($option['objectType']){
                        case self::OT_DATE:
                            $enteredDate = WPDEVHUB_DRC_Utilities::getFromArray($option['objectName'], $requestVars, 0);
                        ...
                        case self::OT_NUMERIC:
                            switch ($option['formType']){
                                case self::ET_MENU_STATUS:
                                    $value = WPDEVHUB_DRC_Utilities::getFromArray($option['objectName'], $requestVars, 0);
                                    if(!is_numeric($value) || empty($value)){
                                        $value=WPDEVHUB_DRC_StandardObjectRecord::STATUS_ACTIVE;
                                    }
                                    $processValue = true;
                                    break;
                                default:
                                ...
                                case self::ET_TEXT_ARRAY_ARRAY:
                                    ...
                                    for($i=0;$i<=50;$i++){
                                        ...
                                        foreach($newItem as $k=>$v){
                                            ...
                                            //WPDEVHUB_DRC_Utilities::logMessage("Field String :: ".$fieldString);
                                            if(array_key_exists($fieldString,$requestVars)){
                                                $val = $requestVars[$fieldString];
                                                if(strlen($val)>=1){
                                                    $val = str_replace("\\", "", $val);
                                                    $newlyMadeItem[$k]=$val;
                                                    //WPDEVHUB_DRC_Utilities::logMessage("Newly Made Item: ".print_r($newlyMadeItem, true));
                                                }
                                            }
                                        }
                        ...
                        case self::OT_ARRAY:
                            switch ($option['formType']){
                                ...
                                case self::ET_ITEM_ZONE_PICKER:
                                    ...
                                    WPDEVHUB_DRC_Zone::removeZonesForItem($object->id, $allZones);
                                    WPDEVHUB_DRC_Zone::addZonesForItem($object->id, $zones);
                                ...
                                case self::ET_TEXT_ADDITIONAL:
                                    ...
                                    $value = $items;
                                    //WPDEVHUB_DRC_Utilities::logMessage("ITEMS: ".print_r($items));
                                    break;
                                case self::ET_DPM_ANSWER_CHOICE_PICKER:
                                    if(is_array($option['formOptions'])){
                                        foreach($option['formOptions'] as $key=>$value){
                                            ...
                                            if(array_key_exists($fieldString, $requestVars)){
                                                ...
                                                $status = WPDEVHUB_DRC_StandardObjectRecord::STATUS_INACTIVE;
                                                if(array_key_exists($chkString,$requestVars)){
                                                    $value = $requestVars[$chkString];
                                                    if($value == 'checked' || $value=='on'){
                                                        $status = WPDEVHUB_DRC_StandardObjectRecord::STATUS_ACTIVE;
                                ...
                                case self::ET_DTT_ITEMS:
                                    ...
                                    foreach($requestVars as $name=>$value){
                                        ...
                                        if(strpos($name, $string) !== false){
                                            ...
                                            // Get the values
                                            $title = WPDEVHUB_DRC_Utilities::getFromArray($rowKey.'_title', $requestVars);
                                            $mediaId = WPDEVHUB_DRC_Utilities::getFromArray($rowKey.'_mediaId', $requestVars);
                                            $contents = WPDEVHUB_DRC_Utilities::getFromArray($rowKey.'_contents', $requestVars);

                                            $list = new WPDEVHUB_DRC_TopTen_Item();
                                            $list->title = $title;
                                            ...
                                            //WPDEVHUB_DRC_Utilities::logMessage("Finished with an Item: ".print_r($list, true));
                        ...
                        case self::OT_STRING:
                            switch ($option['formType']){
                                case self::ET_PASSWORD:
                                    $password = WPDEVHUB_DRC_Utilities::getFromArray($option['objectName'], $requestVars, "");
                                    ...
                                    break;
                                default:
                                    $value = WPDEVHUB_DRC_Utilities::getFromArray($option['objectName'], $requestVars, "");
                                    $value = str_replace("\\", "", $value);
                                    $value = trim($value);
                                    $processValue = true;
                                    //WPDEVHUB_DRC_Utilities::logMessage("Common Save Editor TEXT: Value:".print_r($value,true));
                                    break;
                            }
                            break;
                        case self::OT_BOOLEAN:
                            ...
                            if(array_key_exists($option['objectName'], $requestVars)){
                                ...
                                if($value=='checked' || $value=='selected' || $value=='on' || $value=='1'){
                                    WPDEVHUB_DRC_Utilities::logMessage("Settings Save: Array Key Exists Value is On");
                                    $value = true;
                                }else{
                                    WPDEVHUB_DRC_Utilities::logMessage("Settings Save: Array Key Exists Value is OFF");
                                    $value = false;
                                    $processValue = true;
                                }
                            }else{
                                //Then it means it was a checkmark and was not selected...  will not be included in responseVars
                                WPDEVHUB_DRC_Utilities::logMessage("Settings Save: Array Key Does Not Exist");
                                $value = false;
                                $processValue = true;
                            }
                            break;
            ...
            $object->save();
            WPDEVHUB_DRC_Utilities::addUserMessage('Save Successful');
    ...
    public static function renderAjaxInsertTodaysDate($formIds){
        ...
        foreach($formIds as $formId){
            $html .= 'jQuery(\'#'.$formId.'\').val(\''.WPDEVHUB_DRC_StandardObjectRecord::formatJQueryDate(time()).'\');return false;';
        }
