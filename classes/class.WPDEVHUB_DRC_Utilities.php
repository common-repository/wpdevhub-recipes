 ...
 */
class WPDEVHUB_DRC_Utilities{

    const USER_MESSAGES_KEY = "WPDEVHUB_CONST_DRC_USER_MESSAGES_COOKIE";
    ...
    public static function buildDatabaseVersionString(){
        return WPDEVHUB_CONST_DRC_SLUG . '-db-version';
    }
    ...
    public static function checkForUpgrade($currentVersion, $upgradeCallback){
        global $wpdb;
        $installedVersion = WPDEVHUB_DRC_Utilities::getInstalledVersionNumber();
    ...
    public static function isPluginEnabled(){
        return WPDEVHUB_DRC_StandardSetting::getSetting('plugin_enabled');
    }
    ...
    public static function isPluginPro(){
        if(strtolower(WPDEVHUB_CONST_DRC_LEVEL) == 'pro'){
            return true;
        }
        return false;
    }
    ...
    public static function getBasePluginFile(){
        ...
        return WPDEVHUB_CONST_DRC_SLUG."/index.php";
    }
    ...
    public static function renderPage(){
        ...
        include WP_PLUGIN_DIR.'/'.WPDEVHUB_CONST_DRC_FOLDER.'/pages/'.$plugin_page.'.php';
    }
    ...
    public static function buildPageSlug($page){
        return WPDEVHUB_CONST_DRC_SLUG.'-'.$page;
    }
    ...
    public static function ajaxRegisterPublicHandlers($handlers, $callback){
        WPDEVHUB_DRC_Utilities::logMessage("Inside ".__CLASS__."::".__FUNCTION__);
    ...
    public static function ajaxProcessHandler($handlers){
        ...
        WPDEVHUB_DRC_Utilities::logMessage("inside ajaxProcessHandler");

        // Route the call as appropriate
        $response = '';
        $action = WPDEVHUB_DRC_Utilities::getRequestVarIfExists('action');
    ...
    public static function buildAppClassName($className){
        return $className;
        //$newClassName = $className . '_' . strtoupper(WPDEVHUB_CONST_DRC_APP_CODE) . '_' . strtoupper(WPDEVHUB_CONST_DRC_PLUGIN_LEVEL);
        //return $newClassName;
    }
    ...
    public static function buildHeader($options=array()){
        ...
        $html .= '<div id="drc-wrapper" class="drc-wrapper">';   // Wrapper for all our pages
    ...
    public static function buildStandardObjectPage($objectClassname){
        ...
        ///////////////////////  Editor DISPLAY  ///////////////////////////
        echo WPDEVHUB_DRC_StandardEditor::buildPageTemplate($objectClassname);
        ...
        if(empty($id)){
            ...
            echo WPDEVHUB_DRC_StandardManager::buildManagerTable($rows);
        }
    ...
    public static function logMessage($msg){
        error_log("[".WPDEVHUB_CONST_DRC_APP_CODE."] ".$msg);
    }
    ...
    public static function displayAllUserMessages(){
        ...
        $html = '';
        if(!empty($messages)){
            $html .= '<div id="drc-user-messages-tmp" style="display:none;">';
            foreach($messages as $message){
                $rand = rand(100000,999999);
                $html .= '<div class="drc-user-message" id="dimbal_user_message_'.$rand.'" onclick="dimbalUserMessages.remove('.$rand.')">'.$message.'</div>';
            }
            $html .= '</div>';
        }
    ...
    public static function convertMixedColorValueToRgb($color){
        ...
        $newValue = WPDEVHUB_DRC_Utilities::colorNameToHex($color, false);
        if(!empty($newValue)){
        ...
        $newValue = WPDEVHUB_DRC_Utilities::hex2RGB($color);
        if(empty($newValue)){
    ...
    public static function ensureIsBoolean($booleanToTest){
        //WPDEVHUB_DRC_Utilities::logMessage("Bool To test [$booleanToTest]");
        if(empty($booleanToTest)){
            //WPDEVHUB_DRC_Utilities::logMessage("Returning FALSE - 1");
            return false;
        }elseif($booleanToTest === 0){
            //WPDEVHUB_DRC_Utilities::logMessage("Returning FALSE - 2");
            return false;
        }elseif($booleanToTest === "0"){
            //WPDEVHUB_DRC_Utilities::logMessage("Returning FALSE - 3");
            return false;
        }elseif(strtolower($booleanToTest) === "false"){
            //WPDEVHUB_DRC_Utilities::logMessage("Returning FALSE - 4");
            return false;
        }
        //WPDEVHUB_DRC_Utilities::logMessage("Returning TRUE");
        return true;
    }
    ...
    public static function getThumbnailHtmlForMediaId($mediaId, $classes="", $styles=array()){
        ...
        $styleString = WPDEVHUB_DRC_Utilities::buildStyleStringFromArray($styles);
        if(!empty($src)){
    ...
    public static function extractCommonWords($string, $stop_words, $max_count = 5) {
        ...
        arsort($frequency);
        //WPDEVHUB_DRC_Utilities::logMessage("FREQUENCY ARRAY SORTED: ".print_r($frequency,true));

        //Now just provide the keys... although this is now sorted
        $frequency = array_keys($frequency);

        //WPDEVHUB_DRC_Utilities::logMessage("FREQUENCY ARRAY KEYS: ".print_r($frequency,true));
    ...
    public static function generateSalt($key = '', $length = 20, $caseSensitive = true) {
        ...
        // Removing use of urandom
        for ($i = 0; $i < $length; $i++) {
            $num = rand(0,$max);
            $salt .= substr($chars, $num, 1);
        }
    ...
    public static function ajaxValidateSlugHandler(){
        ...
        $classname = WPDEVHUB_DRC_Utilities::getRequestVarIfExists("class_name");
        $slugText = WPDEVHUB_DRC_Utilities::getRequestVarIfExists("slug_text");
        $elemId = WPDEVHUB_DRC_Utilities::getRequestVarIfExists("elem_id");
        $objectId = WPDEVHUB_DRC_Utilities::getRequestVarIfExists("object_id");
        ...
        if(!empty($objectId)){
            ...
            if(!empty($object)){
                $slugTest = WPDEVHUB_DRC_Utilities::buildSlug($slugText);
    ...
    public static function getFormattedBasicAlignmentString($id){
        ...
        return WPDEVHUB_DRC_Utilities::getFromArray($id, $collection, '');
    }
    ...
    public static function migrateTableNameIfNeeded($oldTableName, $newTableName){
        ...
        global $wpdb;
        $migrationKey = "wpdevhub_drc_tbn_migr_f_".$oldTableName."_t_".$newTableName;

        $value = WPDEVHUB_DRC_StandardSetting::getSetting($migrationKey);
        if(!empty($value)){
        ...
        WPDEVHUB_DRC_StandardSetting::saveSetting($migrationKey, true);
    ...
    public static function migratePostTypesIfNeeded($oldType, $newType){
        ...
        global $wpdb;
        $migrationKey = "wpdevhub_drc_cpt_migr_f_".$oldType."_t_".$newType;
    ...
    public static function migrateTaxonomyTypesIfNeeded($oldType, $newType){
        global $wpdb;
        $migrationKey = "wpdevhub_drc_tax_migr_f_".$newType."_t_".$oldType;
        ...
        WPDEVHUB_DRC_Utilities::logMessage($sql);
        $wpdb->query($sql);
    ...
    public static function migrateMetaBoxDataIfNeeded($oldType, $newType){
        global $wpdb;
        $migrationKey = "wpdevhub_drc_cptm_migr_f_".$newType."_t_".$oldType;
